...
public class ReportController {
    ...
    public Result exportBusinessReport(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String reportDate = (String) result.get("reportDate");//当前日期
            ...
            List<Map> hotSetmeal = (List<Map>) result.get("hotSetmeal");//热门套餐(前四个)

            List<Map> hostSetmeal = (List<Map>) result.get("hostSetmeal");

            HttpSession session = request.getSession();
            ServletContext application = session.getServletContext();
            //获得Excel模板文件绝对路径
            String temlateRealPath = application.getRealPath("template") + File.separator + "report_template.xlsx";

            //读取模板文件创建Excel表格对象
            XSSFWorkbook workbook = new XSSFWorkbook(new FileInputStream(new File(temlateRealPath)));

            XSSFSheet sheet = workbook.getSheetAt(0);
            XSSFRow row = sheet.getRow(2);//第二行
            row.getCell(5).setCellValue(reportDate);//第五列日期
